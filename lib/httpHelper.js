...
/***
 * Looks through channels array and groups array for location returns if the location is in either array
 * @param channels
 * @param groups
 * @param location
 * @returns {boolean}
 */
exports.verifyAccess = function verifyAccess(channels, groups, location) {

    if (channels && groups && location) {
        var index = -1;
        console.log("Checking channels: %s groups: %s for location %s", channels.toString(), groups.toString(), location);
        try {
            if (location.charAt(0) == "G") {
                index = groups.indexOf(location);
            } else if (location.charAt(0) == "C") {
                index = channels.indexOf(location);
            }
        } catch (e) {
            console.error(e);
            index = -1;
