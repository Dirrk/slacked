...
var test_values_inArray = [
    ["#test-test", "drada", 123456789, "Hello I am here Test 7"],
    ["#test-test", "drada", 123456789, "Hello I am here Test 5"],
    ["#test-test", "drada", 123456789, "Hello I am here Test 6"]
];
    ...
    async.eachSeries(profiles,
         ...
         function (profile, next) {
             ...
             if (!data || !data.length || data.length < 1) {
                 ...
                 var sqlQuery = "INSERT INTO locations (locationId, subscribed, name, isChannel, userId) VALUES ? ON DUPLICATE KEY UPDATE name = values(name)";
            ...
            openSQLConnection(function (conn) {
               if (conn) {
                   ...
                   var sqlQuery = "SELECT locations.locationId,locations.lastIndex,locations.name,profiles.userId, profiles.slackToken FROM slacked.locations INNER JOIN profiles ON profiles.userId = locations.userid WHERE locations.subscribed=1 AND locations.isChannel=0";
...
function groupsArrayFromProfile(profile) {
    ...
    for (var i = 0; i < profile.groups.length; i++) {
        ...
        ret.push([profile.groups[i].locationId, subscribed, profile.groups[i].name, isChannel, userId]);
    }
