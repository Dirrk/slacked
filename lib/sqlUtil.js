    ...
    if (data && util.isArray(data)) {
        ...
        openSQLConnection(function (conn) {
           ...
           if (conn) { // execute query
               conn.query(newHistoryQuery, [arr], function (err) {
                   conn.release();
    ...
    try {
        ...
        openSQLConnection(function (conn) {
            ...
            if (conn) {
                ...
                conn.query(updateIndexQueryString, [lastIndex, locationId], function (err, results) {
                    conn.release();
                   if (err) {
    ...
    if (!(locations.length >= 1)) {
        ...
        openSQLConnection(function (conn) {
           if (conn) {
               ...
               conn.query(sqlQuery, [locations], function (err, data) {
...
exports.addUsersToDB = function addUsersToDB(users, callback) {
    ...
    openSQLConnection(function (conn) {
        ...
        if (conn) {
            ...
            conn.query(sqlQuery, [userArray], function (err) {
               conn.release();
                callback(err);
            });
