        ...
        function (err, data) {
            if (!err) {
                if (data && data.length && data.length > 0) {
                    me.locations = data;
                } else if (data && data.channels && data.groups) {
                    me.locations = sUtil.locationsToArray(data);
                } else {
...
function startWorkerJob() {
    ...
    setInterval(me.updateLocatons, me.delay);
...
function parseRTM(message) {
    ...
    switch (message.type) {
        ...
        case "message":
            ...
            ret.msg.data[0] = {
                dateTimeStamp: sUtil.slackTStoJSTS(message.ts).getTime(),
                user: message.user || message.username || null,
                text: message.text || "",
                location: message.channel
            };
            ret.msg.lastIndex = sUtil.slackTStoJSTS(message.ts).getTime();
            break;
        default:
            ret.action = false;
    }
