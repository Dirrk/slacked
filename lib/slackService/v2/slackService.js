...
function ServiceRunner(apiToken) {
    ...
    self.__restart = function () {
        if (self.worker) {
            self.worker.removeAllListeners();
        }
        console.error("Restarting slackService because of a failed event");
        self.worker = null;
        setTimeout(
            function () {
                startTasks(self);
    ...
    // Todo is this required to be a
    console.log("Setting up service runner");

    self.handleHistory = handleChannelData;
}
...
function userUpdateTask(self, next) {
    ...
    slackRequest(config, function (err, data) {
        ...
        if (!err && data.members) {
            ...
            var tmp = mergeObjectsById(self.usersById, parseUserData(data));
...
function locationUpdateTask(self, next) {
    ...
    async.waterfall(
      ...
      function (err, self, results) {
          ...
          next(err, self, hashToArray(results));
      }
    );
};
...
function combineLocationsAndUpdateSQL(self, channels, groups, next) {
    ...
    sqlHelper.getAllLocations(function (err, data) {
        ...
        if (!err && data && data.length) {
            ...
            var locationsHashMap = mergeObjectsById(self.locationsById, self.channelsById).values;
            locationsHashMap = mergeObjectsById(locationsHashMap, self.groupsById).values;
            self.locationsById = locationsHashMap;
            console.log("Location HashMap: %j", locationsHashMap);

            sqlHelper.updateLocationsAndMembership(hashToArray(locationsHashMap), function (err) {
...
function historyUpdateTask(self, locations, next) {
    ...
    if (self.worker != null) {
        console.log("historyUpdateTask worker already created");
    ...
    if (locations && locations.length && locations.length > 0) {
        ...
        console.log("historyUpdateTask - created Worker");
        var historyWorker = new slackWorker(config);
        var historyData = historyWorker.start(function (err) {
            clearTimeout(t);
            if (err) {
                console.error(err);
            }
        ...
        historyData.on('history', function (history) {
            handleChannelData(self, history);
        });
...
function realTimeMessageTask(self, locations, next) {
    ...
    startRTM(self, function (err, data) {
        if (!err) {
            ...
            var newUsers = mergeObjectsById(self.usersById, bots);
...
function RTMHandler(err, self, locations, url) {
    ...
    if (err) {
        ...
        var config = {
            ...
            locations: locations,
            retrieveLocations: getLocationsForWorker,
            delay: HOURS_4
        };
...
function getLocationsForWorker(cb) {

    // For now I am just going to run the locationUpdateTask
    locationUpdateTask(Service, function (err, self, locations) {
...
// Handlers
function handleChannelData(self, data) {
        ...
        if (err || !data) {
            ...
            if (locationId.charAt(0) === 'G') {
                ...
                if (data && data.groups && data.groups.length) {
                    ...
                    var newGroups = mergeObjectsById(self.groupsById, parseGroupData(data.groups.map(parseLocation)));
                    self.groupsById = newGroups.values;
                    self.locationsById = mergeObjectsById(self.locationsById, self.groupsById).values;
...
exports.getChannels = function getChannels() {
    return Service.channelsById;
};

exports.getGroups = function getGroups() {
    return Service.groupsById;
};

exports.getUsers = function() {
    return Service.usersById;
};
...
function mergeObjectsById(values, newObjects) {
