...
SlackLiveWorker.prototype.checkTimeout = function () {
    ...
    // We are active
    if (self.status == 3) {
        // If we have received no pongs in 90 seconds we have timed out close the socket to restart else ping
        (currentTime - self.stats.lastPong > 90000) ? self.socket.close() : self.ping();
    }
...
function parseRTM(message) {
    ...
    if (message.type == "message") { // If I receive a message
        ...
        } else if (message.user != self.userId) {
        ...
        self.status = 3;
        self.stats.lastPong = message.time;
