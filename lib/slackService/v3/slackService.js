...
function handleStats(data, retries) {

    if (!data || !data.locationId || !data.stats || retries >= 3) {
        return;
    } else if (!retries) {
        retries = 0;
    }
    var serviceStartDate = new Date(data.stats.serviceStarted);
    var pluginStats = slackPluginHandler.getStats() || [];
    for (var i = 0; i < pluginStats.length; i++) {
        data.stats.sentMessages += pluginStats[i].stats.responded;
        data.stats.receivedMessages += pluginStats[i].stats.consumed;
    }
    var message = util.format(
        "*Stats*\n" +
            ...
            "Latency:          *%dms*\n" +
            "Service started:    *%s*\n",
        data.stats.sentMessages,
        data.stats.receivedMessages,
        data.stats.latency,
        util.format(
            "%d/%d/%d %d:%d",
            serviceStartDate.getMonth() + 1,
            serviceStartDate.getDate(),
            serviceStartDate.getYear(),
            serviceStartDate.getHours(),
    ...
    slackRequest(options, function (err, data) {
        if (err || !data || !data.ts) {
            setTimeout(handleStats(data, retries + 1), 1000);
        }
    });
}
