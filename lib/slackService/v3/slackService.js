...
function setupHandlers () {
    ...
    // Command Sent
    // TODO Setup actual command handling
    worker.on('command', handleCommands.bind(self));
...
/***
 * Handle commands from bot (not implemented)
 * @param command
 */
function handleCommands(command) {
    ...
    var self = this;
    try {
        console.log("User: %j", self.usersById[command.userId]);
        console.log("Channel: %j",self.locationsById[command.locationId]);
        console.log("Command (%s) received in channel %s from %s with args: %s",
                    command.command,
                    self.locationsById[command.locationId].name,
                    self.usersById[command.userId].name,
                    command.msg
        );
    } catch (e) {
        console.error(e);
...
function handleKeepActive() {
    ...
    slackRequest('users.setActive', function (err) {
       if (err) {
           console.error(err);
       } else {
