...
function fetchGroups(profile, callback) {
    ...
    slackRequest(url, function(err, message) {
        ...
        if (err) {
            ...
            if (global.appConfig.debug) {
                console.log("Groups(%i) received for user %s",message.groups.length, profile.userId);
                console.log(message);
            }
...
function prepareGroups(cb) {
    ...
    async.eachSeries(
        ...
        function (profile, next) {
             ...
             fetchGroups(profile, function (prof) {
...
function updateAndMergeGroups(tmpProfiles, cb) {
    ...
    if (zombieProfile) {
        ...
        fetchGroups(zombieProfile, function (prof2) {
            zombieProfile = prof2;
...
exports.getGroups = function getGroupsForProfile(userId) {
